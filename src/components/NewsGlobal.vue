<template lang="">
  <div class="category">
    <div class="category-buttons">
      <button @click="news" :class="{ active: activeCategory === '' }">전체</button>
      ...
      <button @click="news2('technology')" :class="{ active: activeCategory === 'technology' }">기술</button>
...
export default {
  ...
  created() {
    this.news(); // 기본적으로 전체 뉴스 로드
  },
  methods: {
    async news() {
      this.activeCategory = ""; // 전체 카테고리 활성화
      try {
        const api = await axios.get(`https://server-woad-theta.vercel.app/news`);
        this.titleData = api.data.data[0].title_ko;
    ...
    async news2(category) {
      ...
      try {
        const api = await axios.get(
          `https://server-woad-theta.vercel.app/news/category?category=${category}`
        );
